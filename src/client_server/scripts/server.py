...
from client_server.srv import service , serviceResponse
...
def callback(req):
    ...
    if req.temp == None and req.pre == None and req.hum== None:
        ...
        if req.temp >= 10 and req.temp <= 100:
            rospy.loginfo("The received temperature data is %x ", req.temp)
        ...
        if req.pressure >= .95 and req.pressure <= 1.2:
            rospy.loginfo("The received pressure data is %x ", req.pre)
        ...
        if req.humidity >= .7 and req.humidity <= .95:
            rospy.loginfo("The received humidity data is %x ", req.hum)
...
if __name__ == "__main__":
    ...
    rospy.Service("my_service", service, callback)
    rospy.spin()   